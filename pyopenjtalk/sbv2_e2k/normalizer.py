#from https://github.com/tsukumijima/Style-Bert-VITS2/tree/fork/style_bert_vits2/nlp/japanese
...
from pyopenjtalk.sbv2_e2k.romkan import to_katakana
from pyopenjtalk.sbv2_e2k.symbols import PUNCTUATIONS
...
__SYMBOL_REPLACE_MAP = {
    ...
    "，": ",",
    #"。": ".",
    "。": "。",# pyopenjtalk での形態素解析処理で重要なので正規化後も残す
    "！": "!",
    "？": "?",
    "\n": ".",
    "．": ".",
...
def __convert_english_to_katakana(text: str) -> str:
    ...
    def try_split_convert(word: str) -> str | None:
        ...
        for pos in positions:
            ...
            # 上記はsbv2側の使用　pyopenjtalk-agplではカタカナマップから辞書を作っているのでそのまま返す
            # print(
    ...
    def process_english_word(word: str, enable_romaji_c2k: bool = False) -> str:
        ...
        if number_match:
            ...
            if base_katakana:
                ...
                if number_katakana:
                    return base_word.lower() + number_katakana
